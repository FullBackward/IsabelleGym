    ...
    *,
    ...
    async with IsabelleGymAsyncClient(base_url=server_url, timeout=timeout) as client:
        try:
            ...
            # NOTE: do NOT call enter_theory here.  The header step
            # already contains "theory X imports … begin" which
            # transitions the REPL into the theory context.  Calling
            # enter_theory first sends an extra edit that races with the
            # header step's "theory" command in the async Isabelle
            # document model, occasionally producing
            #   "Bad context for command \"theory\""
