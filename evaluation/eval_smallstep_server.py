...
# Broader command starters so theorem/proof text can be stepped command-by-command.
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Stepwise benchmark via the IsabelleGym server, aligned with the local stepwise evaluator."
    )
    ap.add_argument("--corpus", required=True, type=Path)
    ap.add_argument("--server", default="http://localhost:8000")
    ap.add_argument("--timeout", type=int, default=300)
    ap.add_argument("--field", default="HOL-Analysis")
    ap.add_argument("--output", type=Path, default=Path("smallstep_server_stepwise_aligned.json"))
        ...
        for thy_file in files:
            ...
            try:
                ...
                resp = session.post(
                    ...
                    json={"theories": imports, "field": args.field},
                    timeout=args.timeout,
                )
                resp.raise_for_status()
                session_id = resp.json()["session_id"]

                enter_resp = session.post(
                    f"{base}/api/v1/sessions/{session_id}/enter_theory/{theory_name}",
                    timeout=args.timeout,
                )
                enter_resp.raise_for_status()
                ...
                for idx, (kind, command) in enumerate(command_stream):
                    ...
                    try:
                        resp = session.post(
                            ...
                            json={"command": command, "timeout": args.timeout},
                            timeout=args.timeout,
                        )
                ...
                if session_id is not None:
                    try:
                        session.delete(f"{base}/api/v1/sessions/{session_id}", timeout=args.timeout)
                    except Exception:
                        pass
    ...
    summary = {
        "tool": "isabellegym-server-smallstep",
        "benchmark_kind": "server_smallstep_verification_benchmark_stepwise_aligned_local",
        "server": args.server,
        "field": args.field,
